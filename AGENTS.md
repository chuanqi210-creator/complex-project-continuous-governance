...
- Put the integrated Complex maintenance cycle in `docs/self-optimization.md`; do not create a new mechanism for maintenance procedure.
...
- For long work, define a project-specific time appetite, next stage result horizon, smallest useful stage artifact, quality/evidence floor, and scope-convergence route. Use observed elapsed time and artifact throughput when available. Deliver a usable increment or decision-grade diagnostic by the horizon; if scope is not converging, preserve quality while closing a thin slice, parking work, changing route, or resetting the appetite. Timebox expiry is not Goal completion.
- At project start or strategic reframe, apply Framework Grilling only to unresolved user-owned decision forks that can materially change Goal, target function, architecture, responsibility boundary, or evaluation. Inspect facts and external basis first; ask one question at a time with a recommendation and consequence map; skip the interview when remaining uncertainty is derivable, reversible, or testable by the AI.
- When optimizing Complex itself, classify the change unit before editing. Keep routine corrections light; for bounded or substantial changes write the falsifiable behavior/evaluation contract first, keep a stable baseline, use progressive evidence rollout, and promote, keep, demote, or roll back from observed outcomes. Continue executable residual evidence work without asking for routine permission.
...
- When those boundaries maintain separate state, use event-triggered cross-boundary state reconciliation. Local records stay authoritative for local facts; one controller publishes a compact global control projection from versioned local state capsules. Trigger it at recovery, phase/stage transition, handoff, topology/dependency change, conflict/staleness, or stage delivery, with a project-specific staleness fallback; do not fan in every ledger on every beat or resolve conflicts by recency alone.
