...
- Use `complex_behavior_kernel` first: restore true state, classify project nature, assign decision rights, set the control plane, run the lightest useful work-plane action, deliver to the right audience, and leave next-route recovery. For continuous projects, the control plane covers direction, authority, state, topology, routing, and stop conditions before local optimization.
...
- For continuous projects, confirm the control plane before choosing local edits: manager responsibility, durable lanes, temporary worker pool, context-reset policy, Beat Router, and stop conditions. Durable lanes are project responsibilities that persist across beats; subagents are short-lived workers and must not be described as long-running threads.
...
- If temporary subagents, parallel review, read-only audit, standing-lane records, or clean-context packets are clearly useful and low-side-effect, activate or record the available topology rather than only recommending it. Ask the user only for responsibility-bearing boundaries such as accounts/API credentials, external writes, publishing, irreversible operations, high-risk claims, or platform-visible persistent resources that truly become user-owned side effects.
...
- When Plan mode is asked to design continuous execution, produce an orchestration contract before business execution: capability preflight, resource taxonomy, control plane, standing lane topology, authority/platform-boundary status, manager/worker split, Beat Router, and stop conditions. Do not call a short-lived subagent a user-visible long-running Codex thread.
