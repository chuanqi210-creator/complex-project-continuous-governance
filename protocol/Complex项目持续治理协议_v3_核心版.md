...
Complex Runtime Kit / 持续治理运行时套件是本协议的落地层，而不是新的核心协议。新项目可以用仓库中的 `templates/` 建立 state、evidence、decision、search、question、loop 和 delivery 记录，用 `.codex/shared-skills.json` 辅助能力盘点；但核心判断仍以本协议的阶段路由、证据纪律、Loop、评分、授权边界和交付契约为准。Runtime Kit 模板不自动成为 verifier required 字段，只有反复真实失败证明必要时才按反膨胀机制晋升。
...
2. “这事很重要”“按重大项目来”“别再黑箱”：进入 major_project_mode，并保持低摩擦。
...
2. 再读本文件的快速入口、连续任务节拍、工具状态与人工操作判定表，确定是否触发 continuous、major_project、plan-only、goal、manual_action 或 stop。
...
读取后必须做 6 个判断：本轮模式和深度、current_basis、关键词触发的闸门、能力/工具候选、协作拓扑、Goal/Plan/Loop/交付契约。若用户明确说“完全扫描、完整读取、不要漏 Complex 可借鉴部分”，必须在计划或首轮回复中列出采用/暂不采用/需要后续触发的 Complex 组件；否则不得进入业务执行。
...
| 连续节拍、持续推进、一直继续、直到停止、按看版继续 | continuous cycle、round_index、Goal/Plan 状态和每 3 轮复盘节拍 |
| 多线程、长期线程、子代理、总线程、分线程、并行 | `agent_topology_selection_trace`、`persistent_thread_orchestration_contract` 和周期性职责复核 |
| 外部工具、skill、API、数据库、账号、浏览器、Scholar、机构权限 | `capability_discovery_cadence_gate`、外部能力清单和周期性工具复核 |
| Plan 模式、先规划、不要执行、完整规划 | Plan-only 输出，但必须先完成协议理解和组件覆盖检查 |
| Goal 模式、长期目标、loop 工程、版本号、v32/v38、目标漂移 | `goal_refresh_gate`，检查 Codex goal、轮次目标和项目实际版本是否一致 |
...
请按“Complex 项目持续治理协议 v3”先完成 material intake、current_basis 判断、情景识别、行业最佳实践调查、阶段路由、评分和最小闭环设计。
...
1. 读取最近一次机器看版，恢复 current_stage、current_basis、open_questions、decisions、next_route、round_index、active_goal_summary、topology_summary、capability_summary。
2. 先跑 `continuous_cadence_refresh_gate`：若 round_index 为 1、每满 3 轮、阶段/主链/交付对象变化、子线程输出失配、外部工具阻塞、用户新增关键词或版本号变化，则重新评估协作拓扑、能力清单和 Goal 状态。
3. 只选择一个最高杠杆问题处理，优先处理会造成返工、幻觉、低摩擦失败或交付不可落地的问题。
4. 同步 Codex update_plan；用户明确触发连续节拍、直到停止、长期跟踪或 Goal 模式时，必须检查 Codex goal。active goal 用于持续会话，round_goal 写在机器看版和 plan 中。不得把一轮 round_goal 完成误当成整个持续目标结束。
5. 写回协议、日志、代码或交付物中的必要位置。
6. 扫描关键节点，确认修改真实存在且章节、路由、证据、文件路径没有脱节。
7. 更新机器看版：stage_status、round_index、round_goal、decision、evidence/status、topology_refresh_due、capability_refresh_due、goal_refresh_status、next_route、route_reason、manual_action_required。
8. 给用户一个短进度说明；如果没有阻塞且 continuous stop 条件未满足，下一轮从新的 next_route 继续。

`continuous_cadence_refresh_gate` 默认每 3 轮强制复盘一次，用户也可以明确改成每 2 轮或每 5 轮。复盘不等于重开项目，而是回答三件事：
...
3. Codex goal、round_goal、next_route 和项目实际版本是否一致；若出现 v32/v38 这类目标版本漂移，必须刷新目标或降级为 stale_goal，不得继续沿假 goal 推进。
...
| 情况 | 默认动作 | 必须记录 |
| --- | --- | --- |
| 用户给出一次性复杂任务 | 使用 `update_plan`，不创建 goal | `codex_tool_state.plan` |
| 用户明确要求长期持续目标、直到停止、长期跟踪 | 先 `get_goal`，没有 active goal 时 `create_goal` | `codex_tool_state.goal` 和创建理由 |
| 用户触发连续节拍或 Goal 模式 | active goal 代表持续会话，round_goal 代表本轮；每轮开始跑 `goal_refresh_gate` | goal_objective、round_goal、stale_goal_check、refresh_or_keep_reason |
| active goal 与 current_basis / next_route / 版本号不一致 | 不沿用假 goal；完成或降级旧 goal 后创建/记录新目标，或在工具不支持时写 protocol_round_goal | stale_reason、new_goal_summary、migration_note |
...
16. 如果 Codex 的 plan / goal 工具可用，复杂多步任务必须使用工具状态，而不是只在文本里自拟计划和目标。已有 active goal 时先对齐它；用户明确要求长期目标或持续到停止的目标时再创建 goal；阶段推进时同步 update_plan；真正完成、用户停止或严格阻塞时才更新 goal 状态。
...
19. 如果用户明确说这是重大项目，或项目同时具备高返工成本、高公共性/敏感性、多阶段交付、强主观评价、外部权威引用、代码/文档双交付、多人协作中的任意两项，必须进入 major_project_mode。重大项目不等于问用户更多问题，而是必须维护证据矩阵、决策记录、追踪矩阵和低摩擦反馈节奏。
...
24. 对社会实践、调研、教育、公益、课程、产品或组织方案类重大项目，最终报告的重要主张必须能追踪到执行工具和证据字段。没有“报告主张 -> 执行工具 -> 现场材料 -> 结论边界”的链条，不能把漂亮叙事当作可落地成果。
25. 重大报告使用政策、国际组织、论文、新闻或名人大家话术时，必须做 source_link_recency_audit：优先原始来源，记录 current_as_of，区分原文、转载、二手解读和理论解释；链接失效、来源过期或只能支撑解释不能证明事实时，最终表述必须降级。
...
29. 科研、实验、工程计算、财务、数据分析或任何含数字结论的重大项目，必须维护 numeric_calculation_trace：区分实测值、证书/规格值、示范参数和派生计算值，记录单位、公式、复算状态、边界层级和不能外推的结论。
30. 真实项目压力测试、题库扩展、比赛项目分析或重大项目发散，不能默认从当前工作区、目录名称、本地历史文件、当前本地 AI 科研材料或 AI 熟悉领域取样；也不能因为纠偏而排除外部 AI 科研、渲染比赛、建模大赛、实际工程问题、企业工程赛题或偏理工科真实项目。必须先做 live_project_pool_scan，覆盖现实时间所有主要来源类型，包括但不限于 AI/数据/渲染/建模/工程比赛、公开赛题、社会实践、公益志愿、产业命题、开源任务、政策治理、教育教学、传播设计、生命健康、环保双碳、创新创业和真实组织项目渠道，并记录 current_as_of、渠道层级、选择理由和未覆盖偏差。
...
46.1. 每个新任务、连续优化轮次或重大阶段切换，必须先补 `capability_discovery_cadence_gate`：在 `dynamic_stage_controller` 后、进入执行前，显性记录是否完成初始能力盘点；盘点面至少覆盖 local_skills、callable_tools、deferred_tools、plugins_connectors、external_apis_methods，并写清 selected_now、rejected_now、backlog 和下一次重评点。阶段切换、用户新增要求、阻塞/验证失败、外部写入或子代理边界、最终主张前，都必须重新考虑是否需要新 skill/tool/plugin/API/外部方法。连续节拍默认每 3 轮强制复盘一次能力清单，即使没有阻塞也要检查初始工具是否已经不匹配当前阶段；若不匹配，必须改写 selected/rejected/backlog/manual_action 和下一轮重评点。小型本地任务可写 `intentionally_skipped`，但必须说明已有本地工具足够；不强制每次联网或调用 `tool_search`。`skill-creator` 只有在目标是创建或更新可复用 Codex skill 时才选用，否则写入 rejected/backlog。
46.1-补充. `scheduled_topology_capability_review`：连续节拍、长期线程、子代理协作或长期 Goal 运行时，必须建立 round_index 和 review_interval，默认 `review_interval: 3`。每到第 1、3、6、9... 轮，或出现阶段切换、主链变化、子线程输出偏离、工具阻塞、用户新增关键词、版本号变化时，主线程必须重新评估 agent_topology、subagent/thread 职责、外部能力组合和 goal_refresh_status。长期分线程不能永久沿用最初职责；若职责不匹配，必须给出 adjust_scope、pause_or_close、new_context_packet 或 no_longer_needed。
...
46.1-补充. `protocol_onboarding_comprehension_gate`：新项目要求读取 Complex、Auto Research、Complex 项目持续治理协议或类似强约束方法时，必须先安排协议理解时间，再开始业务产出。最低要求是按“Complex 读取顺序与关键词触发”完成扫描：本项目状态和 AGENTS/README、快速入口、连续任务节拍、Stage 0-10、能力发现、子代理/线程、Loop/评分、Plan/Goal、交付拆分、人看版规则、Runtime Kit 模板。输出必须包含 protocol_scan_order、keyword_trigger_map、adopt_now、skip_now、backlog、manual_action 和最大误读风险。若用户说“完全扫描、完整读取、不要漏”，Plan 模式也不能只给泛泛计划；必须先完成上述覆盖检查，再给业务计划。若协议较长或项目高风险，可以让只读子代理专门做协议理解和标题/知识图谱式权重分配；但最终采用与取舍必须由主代理决定。
46.1-补充. `goal_refresh_gate`：连续节拍中必须区分 active_goal、round_goal 和 next_route。active_goal 服务长期会话，round_goal 服务本轮 Loop；一轮完成只结束 round_goal，不自动结束 active_goal。每轮开始检查 active_goal 是否仍匹配 current_basis、next_route、版本号、交付对象和最高杠杆问题；若出现目标文本停留在旧版本、旧需求或旧交付物，例如 active_goal 写 v32 而项目已到 v38，必须标记 stale_goal，完成/降级旧 goal 后创建或记录新目标。若工具不支持修改 goal 文本，则在机器看版写 protocol_round_goal 和 goal_migration_note，不能继续沿假 goal 执行。
...
5. 如果启用了 `dynamic_stage_controller`，handoff 必须记录 mode、stage_depth_budget、route_event、routing_decision_log、subagent_decision 和 route_reason；不能只写“继续优化”。
...
默认原则：先判断本轮应该停在哪个阶段，再决定该阶段要轻跑、标准跑、深跑、并行跑还是回退。用户不需要知道这些字段；当用户只说“重大项目”“继续任务”“优化你自己”时，AI 应自动选择合适深度，并把判断写入机器看版。
...
`gate_activation_matrix` 负责把“重大项目常驻检查”转成动态执行，而不是把所有 guard 全量硬跑。任何 gate 都必须先归类为 always_minimum、risk_based、continuous_only 或 recovery_only；risk_based gate 必须有 activation_predicate 和 minimum_by_mode，跳过时必须写 skip_record_required。若路由表写“默认做”，本控制器仍要说明在当前 mode 下最低做到了哪一层，防止 low friction 被误用为跳过证据，也防止 deep/continuous 自动膨胀成目录式清单。
...
stage_depth_ladder:
  light:
    ...
    forbidden_overhead:
      ...
      - 不为了仪式感调用子代理或创建 Goal
  standard:
    use_when: 普通重大项目、需要交付方案、报告、代码或多步骤执行
    minimum_outputs:
      - current_basis
...
5. 深度选择必须记录 alternatives_considered；尤其当项目同时像小任务、重大项目和长期项目时，要说明为什么没有选择其他深度。
...
```yaml
stage_card:
  purpose: 在 current_basis 和最佳实践约束下生成多个可比较方向。
  entry_triggers:
    - 问题空间未收束
    - 用户还在表达愿景、偏好或新增想法
    - 现有方案缺抓手、故事性或落地性
  ...
  close_condition: 候选足够多样，且每个候选都能说明依据、风险和最小验证
```
...
stage_card:
  ...
  required_outputs:
    ...
    - capability_discovery_cadence_gate.periodic_reconsideration
    - 候选 skill / 工具 / 方法
    - 适用阶段、风险、是否安装、是否只借鉴
    - 推荐调用顺序
...
stage_card:
  ...
  dynamic_shortcuts:
    ...
    - 已有 active goal 时先对齐，不另造更容易完成的小目标
  route_back_triggers:
    - Goal 变成任务清单
    - Goal 与用户最新意图或 current_basis 冲突
    - 终止条件不可审计
  subagent_fit: 不允许子代理拥有或更新主目标；reviewer 只能审 Goal 是否一致和可验收
  close_condition: Goal 能长期约束项目，且工具状态和文档状态一致
```

工具规则：

1. 只有用户明确要求建立长期目标、持续迭代直到停止，且没有 active goal，才使用 Codex goal 工具创建。
2. 不得因为阶段产物完成、预算接近或自己想停下就把 goal 标记 complete；只有完成性审计证明原目标全满足时才能 update_goal complete。
...
stage_card:
  ...
  dynamic_shortcuts:
    - 小任务只跑 must_pass
    - 重大项目跑 must_pass + risk_based_checks + artifact_freshness_checks + manual_decision_gates
...
## 8.7 重大项目模式、证据矩阵与决策记录

本规则用于把“复杂项目”升级为可审计、可继承、可复盘的重大项目流程。它借鉴三类成熟做法：需求追踪强调需求、证据和验收之间可追溯；Stage-Gate / 技术评审强调每个阶段必须有进入/退出条件；Architecture Decision Records 强调重大取舍必须记录上下文、选项、决定和后果；系统工程评审强调结论要能回到证据和验证。

### 触发条件

出现任一条件，默认进入 major_project_mode：
...
major_project_mode 不是要求每一步都停下来问用户。它要求每一步的关键判断可追踪、可降级、可被用户低成本改方向。
...
重大项目必须能从用户诉求追踪到最终交付物，不允许“中间看起来很努力，最终不知道服务了哪个要求”。
...
涉及真实项目、比赛项目、题库扩展、跨项目压力测试、行业最佳实践调查或重大项目发散时，必须先建立现实时间项目池，不能默认从当前工作区、目录名称、当前本地 AI 科研材料、上一轮主题或 AI 熟悉领域取样，也不能把“避免只看 AI 科研”误解成“排除外部 AI/理工真实项目”。
...
重大项目追踪必须先路由后启用，避免把所有经验库条目套给每个项目。
...
7. 如果处于重大项目，是否维护 evidence_matrix、decision_log、traceability_matrix。
...
8. 如果项目已进入 major_project_mode，但没有证据矩阵、决策记录或追踪矩阵，说明只是口头上重大，流程上仍是黑箱；必须回到 8.7 补齐最小治理包。
