...
Complex Runtime Kit / 持续治理运行时套件是本协议的落地层，而不是新的核心协议。新项目可以用仓库中的 `templates/` 建立 state、evidence、decision、search、question、prompt、loop 和 delivery 记录，用 `.codex/shared-skills.json` 辅助能力盘点；但核心判断仍以本协议的阶段路由、证据纪律、Loop、评分、授权边界和交付契约为准。Runtime Kit 模板不自动成为 verifier required 字段，只有反复真实失败证明必要时才按反膨胀机制晋升。
...
2. 明确本轮目标 `round_goal`，长期项目再对齐 `active_goal`。
3. 建立或更新 Plan。
4. 判断证据、能力、协作拓扑和交付对象。
5. 用 5-30 分钟 Loop 或等价小检查验证最大不确定性。
6. 评分并决定 continue、route-back、ask_user、blocked 或 execute。
7. 交付人看版，并留下可恢复的机器记录。
...
读取后必须做 6 个判断：本轮目标和 current_basis、用户需要确认或默认的入口选择、内部工作力度/风险升级、能力/工具候选、协作拓扑、Goal/Plan/Loop/交付契约。内部 `mode` 或 `stage_depth_budget` 可以写入机器看版，但不能把“本轮/深度/普通/重大”等内部词变成用户必须理解的入口。若用户明确说“完全扫描、完整读取、不要漏 Complex 可借鉴部分”，必须在计划或首轮回复中列出采用/暂不采用/需要后续触发的 Complex 组件；否则不得进入业务执行。
...
2. 外部能力边界：是否允许使用网页、浏览器、数据库、账号、API、skill、子代理或其他外部方法；哪些只列候选，哪些需要用户授权。
3. 协作拓扑：先由主线程推进，还是需要临时子代理、长期分线程或并行核验。
4. 推进节拍：先完成单轮并留下 next_route，还是启用连续节拍直到用户停止。
5. 证据和权限边界：是否有隐私、账号、付费、发布、现实世界操作或人工确认限制。

默认问题示例：

```text
我会按 Complex 的目标-计划-Loop-评分-交付循环推进。开始前只确认几件会影响路线的事：是否允许外部工具/账号/API，是否需要子代理或多线程，是否启用连续节拍，以及这次交付主要给谁看？如果你不指定，我先用“主线程单轮推进、只读本地与公开资料、留下下一步、人看版给第三方也能读懂”的安全默认。
```

### Complex 提示词设计前置入口

`complex_prompt_bootstrap_gate`：当用户希望“先扫描 Complex、再为项目设计提示词、之后按提示词推进”时，AI 必须把这一步当成正式入口，而不是普通的 prompt engineering 建议。这个入口的目标是让用户只给出最小请求，也能得到一个项目专用执行契约，再由用户确认后进入业务执行。
...
3. 把用户项目语境转成项目专用执行提示词，提示词内必须包含：项目目标、当前材料、交付契约、能力边界、子代理/线程策略、cadence、round_goal / active_goal 关系、Loop 小循环、评分路由、证据和权限边界、恢复记录方式。
...
`round_execution_prompt` 至少包含：长期目标、当前状态、本轮 `round_goal`、总规划约束、能力边界、协作拓扑、Loop 小循环、评分路由、交付契约和 next_route。每轮 Plan 必须说明：哪些来自总规划，哪些来自上一轮状态，哪些是本轮新增判断。若 Plan 只写当下任务、没有回扣总规划或 active_goal，必须触发 route-back，先补 `round_execution_prompt`。
...
1. 读取最近一次机器看版，恢复 current_stage、current_basis、open_questions、decisions、next_route、round_index、master_prompt_location、active_goal_summary、topology_summary、capability_summary。
2. 先跑 `round_prompt_rehydration_gate`：从已确认总 prompt / active_goal 和最新 state 压缩出本轮 `round_execution_prompt`，明确总规划继承、上一轮状态和本轮新增判断。
3. 再跑 `continuous_cadence_refresh_gate`：若 round_index 为 1、每满 3 轮、阶段/主链/交付对象变化、子线程输出失配、外部工具阻塞、用户新增关键词或版本号变化，则重新评估协作拓扑、能力清单、Goal 状态和 master prompt 是否需要局部 patch。
4. 只选择一个最高杠杆问题处理，优先处理会造成返工、幻觉、低摩擦失败或交付不可落地的问题。
5. 同步 Codex update_plan；Plan 必须基于 `round_execution_prompt`，说明来自总规划、上一轮状态和本轮新增判断的部分。用户明确触发连续节拍、直到停止、长期跟踪或 Goal 模式时，必须检查 Codex goal。active goal 用于持续会话，round_goal 写在机器看版和 plan 中。不得把一轮 round_goal 完成误当成整个持续目标结束。
6. 写回协议、日志、代码或交付物中的必要位置。
7. 扫描关键节点，确认修改真实存在且章节、路由、证据、文件路径没有脱节。
8. 更新机器看版：stage_status、round_index、round_goal、round_execution_prompt_summary、decision、evidence/status、topology_refresh_due、capability_refresh_due、goal_refresh_status、prompt_refresh_status、next_route、route_reason、manual_action_required。
9. 给用户一个短进度说明；如果没有阻塞且 continuous stop 条件未满足，下一轮从新的 next_route 继续。
...
| 情况 | 默认动作 | 必须记录 |
| --- | --- | --- |
| 用户给出一次性复杂任务 | 使用 `update_plan`，建立本轮 `round_goal`；通常不创建长期 active goal | `codex_tool_state.plan`、round_goal |
...
16-补充. Complex 项目的 Goal / Plan / Loop 是基础动作：一次性任务至少要有本轮 round_goal、计划、小闭环或显式 no-op 检查、评分路由和交付契约；连续项目还要维护 active_goal 与 round_goal 的区别。
...
46.1-补充. `core_goal_plan_loop_required`：Complex 不把 Goal、Plan 和 Loop 当作可选模式。任何 Complex 任务都要至少记录本轮 round_goal、当前计划、一个 5-30 分钟 Loop 或明确 no-op 检查、评分路由、交付契约和恢复入口；连续项目再额外维护 active_goal。Codex `create_goal` 只在长期持续目标、直到停止或长期跟踪时使用；一次性任务不创建长期 goal，但不能因此没有本轮目标。
...
46.1-补充. `complex_prompt_bootstrap_gate`：用户要求“扫描 Complex 后设计提示词”“给一个完美 prompt 后再推进”或类似低摩擦入口时，必须先完成协议理解和项目提示词设计，不能直接进入业务产出。输出至少包含 protocol_scan_summary、startup_questions_or_defaults、project_prompt_design_rationale、copy_ready_prompt 和 execution_bridge。`copy_ready_prompt` 必须把本项目目标、材料、交付对象、能力边界、子代理/线程、连续节拍、round_goal/active_goal、Loop、评分路由、证据边界、权限边界和恢复记录写成可复制执行契约。用户确认前只做提示词和流程设计；确认后执行仍受 Complex 主协议、最新用户指令和副作用授权约束，不能把旧 prompt 当成永久目标或免确认授权。
...
90. **round_prompt_missing_gap**：连续节拍新一轮没有先生成 `round_execution_prompt`，就直接进入业务计划或执行，导致 Plan 与 master prompt、active_goal、next_route 脱节。
